...
def process_option(option, filtered_df, codon, current_germline, starting_point, ending_point, igphyml_df_new, pgen_model, expected_mu=None, mrca=None, sd=0.5):
    ...
    if expected_mu is not None and mrca is not None:
        temp_data = {
            ...
            'joint_log_likelihood': new_lhoods[3],
            'tree_log_likelihood': new_lhoods[1],
            'pgen_log_likelihood': new_lhoods[0],
            'expected_mu_likelihood': new_lhoods[2]
            }
    else:
        temp_data = {
            'site': codon//3,
...
def get_updated_germline(starting_germline, starting_cdr3, igphyml_df, pgen_model, starting_point, ending_point, cdr3_only=True, max_iter=100, nproc = 1, quiet = 0, expected_mu=None, mrca=None, sd=0.5):
    ...
    while still_improving and iteration < max_iter:
        ...
        for codon in codon_list:           
            ...
            if mrca is not None and expected_mu is not None:
                ...
                tested_lhoods_expected_mu.append(results[best_index][1][2])
                tested_lhoods_combo.append(results[best_index][1][3])
            else:
                tested_combinations.append(results[best_index][0])
                tested_lhoods_pgen.append(results[best_index][1][0])
                tested_lhoods_tree.append(results[best_index][1][1])
    ...
    if mrca is not None and expected_mu is not None:
        data = {
            ...
            'expected_mu_lhood': tested_lhoods_expected_mu,
...
def process_row(row):
    ...
    if row['mrcas'] == "NULL" or row['centers'] == "NULL":
        mrca = None
        expected_mu = None
    else:
        with open(row['mrcas'], "r") as f:
            mrca = f.read().strip()
        mrca = mrca[starting_point:ending_point]
        with open(row['centers'], "r") as f:
    ...
    pwm = logo_df.pivot(index='site', columns='amino_acid', values='relative_likelihood').fillna(0).infer_objects()
...
if __name__ == '__main__':
    ...
    if params.get("search", "") == "codon":
        ...
        for index, row in index_table.iterrows():
            ...
            values = get_updated_germline(starting_germline, starting_cdr3, igphyml_df, pgen_model, starting_point, ending_point, cdr3_only=True, max_iter=int(params.get("max_iters", 100)), nproc=int(params.get("nproc", 1)), expected_mu=None, mrca=None)
    ...
    elif params.get("search", "") == "codon_depth":
        index_table = pd.DataFrame({
            'clone_ids': clone_ids_list,
            'starting_germline': starting_germlines_list,
            'junction_locations': junction_locations_list,
        ...
        for index, row in index_table.iterrows():
            ...
            try:
                ...
                print(f"Error writing output files: {e}")
    elif params.get("search", "") == "nt_depth":
        index_table = pd.DataFrame({
            'clone_ids': clone_ids_list,
            'starting_germline': starting_germlines_list,
            'junction_locations': junction_locations_list,
