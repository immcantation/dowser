...
#' }
#' @export
plotTrees <- function(tree, nodes=TRUE, tips=TRUE, trait=NULL, tipsize=NULL, 
	data=NULL,	scale=0.01){
    ...
    if(!is.null(data)){
        if(class(data) == "list"){
            ...
            if(length(index) > 1){
                stop("clone", tree$name,
                	" found more than once in list of clone objects")
            }
            data <- data[[index]]
            p <- p %<+% data@data
        }
    ...
    if(!is.null(trait)){
        ...
        p <- p + ggtree::geom_tippoint(aes(color=!!rlang::sym(trait)), 
        	size=tipsize)
    }
