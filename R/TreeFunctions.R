...
# CGJ -- 2/6/23 - may want to delete since pmlpart is trash
# Internal functions to split the data into light chain and heavy chain data.
# returns the data object from the AIRR table that now only contains 
# the desired chain information. 
#
filter_chain_data <- function(clone, index){
  clone@hlgermline <- paste(strsplit(clone@hlgermline,split="")[[1]][index], collapse = "")
  clone@numbers <- clone@numbers[index]
  clone@locus <- clone@locus[index]
  clone@region <- clone@region[index]
  clone@data$hlsequence <- split_by_index(clone, index)
  return(clone)
}
split_by_index <- function(clone, index){
  bob <- c()
  for(i in 1: length(clone@data$hlsequence)){
    bob[i] <- paste(strsplit(clone@data$hlsequence,split="")[[i]][index], collapse = "")
  }
  return(bob)
}
...
#' @param    rep             current bootstrap replicate (experimental)
#'
#' @return  \code{phylo} object created by phangorn::pmlPart with nodes
#'          attribute containing reconstructed sequences.
#' @export
buildPMLPart <- function(clone, seq="hlsequence", sub_model="GTR", gamma=FALSE, asr="seq",
                         asr_thresh=0.05, tree=NULL, data_type="DNA", optNni=TRUE,
                         optQ=TRUE, verbose=FALSE, resolve_random=TRUE, quiet=0, rep=NULL){
  # note quiet and rep are not used here
  data_type <- toupper(data_type)
  asr <- tolower(asr)
  heavy_index = clone@locus == "IGH"
  light_index = clone@locus != "IGH"
  heavy <- filter_chain_data(clone, heavy_index)
  light <- filter_chain_data(clone, light_index)
  # get the other AIRR clone slots
  heavy_seqs <- heavy@data[[seq]]
  light_seqs <- light@data[[seq]] 
  heavy_names <- heavy@data$sequence_id
  light_names <- light@data$sequence_id 
  if(seq == "hlsequence"){
    h_germline <- heavy@hlgermline
    l_germline <- light@hlgermline
  } else if(seq == "sequence"){
    h_germline <- heavy@germline
    l_germline <- light@germline
  } else if(seq == "lsequence"){
    h_germline <- heavy@lgermline
    l_germline <- light@lgermline
  }
  
  heavy_seqs <- append(heavy_seqs, h_germline)
  light_seqs <- append(light_seqs, l_germline)
  heavy_names <- c(heavy_names, "Germline")
  light_names <- c(light_names, "Germline")
  # break up the sequences by nucleotide
  heavy_seqs <- strsplit(heavy_seqs,split="") 
  light_seqs <- strsplit(light_seqs,split="") 
  # make sure that the seq names are the named by sequence id
  names(heavy_seqs) <- heavy_names
  names(light_seqs) <- light_names 
  # inital phyDat
  if(data_type =="DNA"){
    heavy_partition <- phangorn::phyDat(ape::as.DNAbin(t(as.matrix(dplyr::bind_rows(heavy_seqs))))) 
    light_partition <- phangorn::phyDat(ape::as.DNAbin(t(as.matrix(dplyr::bind_rows(light_seqs)))))
  }else{
    heavy_partition <- phangorn::phyDat(ape::as.AAbin(t(as.matrix(dplyr::bind_rows(heavy_seqs)))),
                                        type="AA")
    light_partition <- phangorn::phyDat(ape::as.AAbin(t(as.matrix(dplyr::bind_rows(light_seqs)))),
                                        type="AA")
  }
  # now update the attributes so the index works the way we need
  attr(heavy_partition, "index") <- data.frame(index = attr(heavy_partition, "index"), genes=rep(paste0("heavy_", clone@clone), length(attr(heavy_partition, "index"))))
  attr(light_partition, "index") <- data.frame(index = attr(light_partition, "index"), genes=rep(paste0("heavy_", clone@clone), length(attr(light_partition, "index"))))
  if(is.null(tree)){
    if(seq == "hlsequence"){
      # get the optim.pml output
      dm  <- phangorn::dist.ml(heavy_partition)# find the distances
      ...
      dm  <- phangorn::dist.ml(heavy_partition)
      treeNJ  <- ape::multi2di(phangorn::NJ(dm), random=resolve_random) 
    ...
    # okay now use the partitioned data to get the rest of the info needed for build pml
    if("error" %in% class(partitioned_data)){
      if(verbose){
    ...
    # return the hc tree
    # maybe later do another column with the partitioned trees (list of two)
    tree <- ape::unroot(ape::multi2di(partitioned_data$fits[[1]]$tree))
    tree$tree_method <- paste("phangorn::pmlPart::", sub_model)
    tree$edge_type <- "genetic_distance"
    nnodes <- length(unique(c(tree$edge[,1],tree$edge[,2])))
    tree$nodes <- rep(list(sequence=NULL),times=nnodes)
  }
  ...
  tree$seq <- seq
  if(asr != "none" && data_type=="DNA"){
    seqs_ml <- phangorn::ancestral.pml(partitioned_data$fits[[1]],
                                       type="marginal",return="prob")
    ASR <- list()
    for(i in 1:max(tree$edge)){
      patterns <- t(subset(seqs_ml, i)[[1]])
      attr_index <- attr(seqs_ml,"index")
      if(seq == "sequence"){
        attr_index <- attr(heavy_partition, "index")
      } else if(seq == "lsequence"){
        attr_index <- attr(light_partition, "index")
      }
      pat <- patterns[,attr_index$index]
      if(asr == "seq"){
        thresh <- pat > asr_thresh
        acgt <- c("A","C","G","T")
        seq_ar <- unlist(lapply(1:ncol(pat),function(x){
...
buildRAxML <- function(clone, seq = "sequence", exec, submodel = 'GTRGAMMA', rseed = 28, n_searches = 1,
                       ...
                       asr = TRUE, partition = NULL, rep = 1, dir = NULL, ...){
  ...
  fileConn<-file(paste0(path, "/input_data_", name, ".phy"))
  writeLines(c(paste0(as.character(length(clone_seqs)), " ", 
                      as.character(nchar(clone_seqs[1])))), fileConn)
  for(i in 1:length(clone_seqs)){
    write(paste0(clone_seqids[i], "    ", clone_seqs[i]), file=paste0(path,"/input_data_", name, ".phy"), 
          append = TRUE)
  }
  close(fileConn)
  input_data <- paste0(path,"/input_data_", name, ".phy")
  
  # make the command 
  command <- paste("-m", submodel, "-p", rseed, "-s", 
                   input_data, "-N", n_searches, "-n", name, "-w", path)
  if(!is.null(starting_tree)){
    if(from_getTrees){
      ape::write.tree(starting_tree, paste0(path,"/og_starting_tree.tree"))
    }
  ...
  if(file.exists(paste0("input_data_", name, ".phy.reduced"))){
    stop("Preprocessing broke. Not all noninformative sites by RAxML's definition were removed.")
  }
  if(asr){
    tree <- rerootTree(ape::unroot(ape::read.tree(paste0(path,"/RAxML_bestTree.", name))), "Germline", verbose = 0)
    starting_tree <- paste0(path,"/rerooted_", name, ".tree")
    ape::write.tree(tree, starting_tree)
    input_data <- paste0(path,"/input_data_", name, ".phy")
    command <- paste("-m", submodel, "-p", rseed, "-s", 
                     input_data, "-N", n_searches, "-n", paste0(name, "_asr"),
                     "-f A", "-t", starting_tree, "-w", path)
    if(nproc > 1){
    ...
    asr_tree <- ape::read.tree(paste0(path,"/RAxML_nodeLabelledRootedTree.", name, "_asr"))
    ...
    ASR <- readLines(paste0(path,"/RAxML_marginalAncestralStates.", name, "_asr"))
    ASR_df <- c()
  ...
  closeAllConnections()
  if(rm_files){
    two_remove <- c(input_data, paste0(path,"/RAxML_bestTree.", name), paste0(path,"/RAxML_info.", name),
                    paste0(path,"/RAxML_log.", name), paste0(path,"/RAxML_parsimonyTree.", name), paste0(path,"/RAxML_result.", name))
    if(asr){
      two_remove <- append(two_remove, c(paste0(path,"/RAxML_marginalAncestralProbabilities.",name,"_asr"), 
                                         paste0(path,"/RAxML_nodeLabelledRootedTree.",name,"_asr"),
                                         paste0(path,"/RAxML_marginalAncestralStates.",name,"_asr"),
                                         starting_tree,
                                         paste0(path,"/RAxML_info.",name,"_asr")))
    }
    if(from_getTrees){
