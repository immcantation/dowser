...
reconIgPhyML <- function(file, modelfile, cloneid, 
    ...
    nproc=1, quiet=0, rm_temp=FALSE, rm_dir=NULL, 
    ...
    if(mode == "switches"){
        ...
        if(is.null(palette)){
            palette <- getPalette(states,"Dark2")
        }
...
buildPhylo <- function(clone, trait, dnapars, temp_path, verbose=FALSE, 
	rm_temp=TRUE){
    stop("Dowser is not yet compatible with buildPhylipLineage.")
    phylo <- tryCatch({
        alakazam::buildPhylipLineage(clone, dnapars, rm_temp=rm_temp, phylo=TRUE,
        	verbose=verbose,
            temp_path=temp_path)},
...
#' values specified in the \code{trait} option. It returns a list of \code{ChangeoClone} 
...
#' @param    trait        data column to analyze using phylogenetic techniques. 
#'                        Identical sequences sharing this trait will be un-collapsed.
...
formatClones <- function(data, id="sequence_id", seq="sequence_alignment", 
                             ...
                             jcall="j_call", junc_len="junction_length", 
                             clone="clone_id", mask_char="N",
...
rerootGermline <- function(tree, germid, resolve=FALSE){
    ...
    tree$edge.length[mrcaedge] <- tree$edge.length[mrcaedge] + 
    tree$edge.length[rootedge]
...
#' @param    data      list of \code{changeoClone} objects
...
getTrees <- function(data, trait=NULL, id=NULL, dir=NULL, modelfile=NULL,
    dnapars=NULL, igphyml=NULL, trees=NULL, nproc=1, quiet=0, rm_temp=FALSE,
    ...
    if(!is.null(igphyml)){
        ...
        if(is.null(modelfile)){
            ...
            modelfile <- makeModelFile(states, file=file.path(dir,
            	paste0(id, "_modelfile.txt")))
        }else{
            states <- readModelFile(modelfile)
        }
    }
    ...
    if(!is.null(igphyml)){
        ...
        mtrees <- reconIgPhyML(file, modelfile, igphyml=igphyml, 
            ...
            rm_temp=rm_temp, rm_dir=rm_dir, states=states, 
...
#' Create a bootstrap distribution for clone sequence alignments, and estimate trees
#'  for each bootstrap replicate.
#' 
#' \code{bootstrapTrees} Phylogenetic bootstrap function.
#' @param    data       list of \code{changeoClone} objects
#' @param    bootstraps number of bootstrap replicates to perform
#' @param    trait      trait to use for parsimony models (required if \code{igphyml}
#'                      specified)
#' @param    dnapars    location of phylip dnapars executible if desired (not yet 
#'                      supported)
#' @param    igphyml    location of igphyml executible if trait models desired 
#'                      (optional)
#' @param    id         unique identifer for this analysis (required if \code{igphyml}
#'                       or \code{dnapars} specified)
#' @param    dir        directory where temporary files will be placed (required
#'                       if \code{igphyml} or \code{dnapars} specified)
#' @param    modelfile  file specifying parsimony model to use
#' @param    trees      tree topologies to use if aready available (bootstrapping
...
bootstrapTrees <- function(data, bootstraps, nproc=1, trait=NULL, dir=NULL, 
    ...
    quiet=0, rm_temp=FALSE, palette=NULL, resolve=2, rep=NULL,
    ...
    if(is.null(rep)){
        ...
        if(!is.null(igphyml)){
            ...
            switches <- reconIgPhyML(file, modelfile, igphyml=igphyml, 
                ...
                quiet=quiet, rm_temp=FALSE, rm_dir=NULL, nproc=nproc,
            ...
            permuteAll <- reconIgPhyML(file, modelfile, igphyml=igphyml, 
                ...
                quiet=quiet, rm_temp=rm_temp, rm_dir=rm_dir, nproc=nproc,
                resolve=resolve, rseed=rseed)
