...
getTimeTrees <- function(clones, template, beast, dir, id, time,  
        ...
        burnin=10, trait=NULL, resume_clones=NULL, nproc=1, adapt_operators=FALSE,
        quiet=0, rm_temp=FALSE, include_germline=TRUE, seq="sequence", 
...
#  }
  ...
  trees <- tryCatch(buildBeast(data,
                            ...
                            trees=trees,
                            adapt_operators=adapt_operators,
                            ...
                            ),error=function(e)e)
...
buildBeast <- function(data, beast, time, template, dir, id, mcmc_length = 1000000, 
  ...
  if (iterate && is.null(iter)) {
    stop("If iterate is TRUE, iter must be specified")
  }

  # Create the XML file using the current template and include germline
  # Don't remake if we're iterating and this is not the first iteration
  if(!iterate || (iterate  && iter == 0)){
...
#' Adapt the operator weights for BEAST runs by examining ESS values after a short run.
#' 
#' @param    data     a list of \code{airrClone} objects 
#' @param    template   XML template
#' @param    beast      location of beast binary directory (beast/bin)
...
adaptOperators <- function(data, beast, time, template, dir, id, xml_filepath=NULL,
  ...
  capture <- parallel::mclapply(1:length(data), function(x) {
    ...
    console_loglines <- readLines(console_logfile)
    operator_logstart <- grep("^Operator\\s+", console_loglines)[length(grep("^Operator\\s+", console_loglines))]
    operator_logend <- grep("^\\s*Tuning:", console_loglines)[length(grep("^\\s*Tuning:", console_loglines))] - 2
    operator_loglines <- console_loglines[operator_logstart:operator_logend]
    operator_loglines <- gsub("Try.*[0-9]+", "", operator_loglines)
    operator_loglines <- gsub(" $", "", operator_loglines)
    operator_loglines <- gsub("\\#", " ", operator_loglines)
    operator_log <- read.table(text=operator_loglines, header=TRUE, fill=TRUE, stringsAsFactors=FALSE)

    # Decide which operators to adapt (shrink weights)
    exclude_items <- exclude_logs
    exclude_pattern <- paste0("^(", paste(exclude_items, collapse = "|"), ")")
    adapt_log <- log[!grepl(exclude_pattern, log$item), ]

    # Collapse grouped parameters and set ESS to min(ESS) for the group
    adapt_log_collapse <- adapt_log %>%
    ...
    for (row in 1:nrow(adapt_log_collapse)) {
      ...
      weight_line_num <- operator_block$lines[grepl("weight", operator_block$operator)]
