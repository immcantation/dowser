...
Plotting trees with time values on the tips is simple. Just specify the time column as the `tips` value in `plotTrees`. This is detailed in the [Plotting Trees Vignette](https://dowser.readthedocs.io/en/stable/vignettes/Plotting-Trees-Vignette/). There are many ways to customize these plots. Personally, I (Ken) typically use the following options for visualizing longitudinal data:
