...
## Incorporating switching contraints
...
`makeModelFile` produces a physical file - you can open it with a text editor to inspect it. All runs of `findSwitches` create a model file, it's just normally not seen by the user and contains no constraints.
...
Performing the SP test is the same as before, just specify the model file created earlier. Note that no switches occur in directions that are forbidden by our model. If you try this without specifying the model file, you'll likely get many biologically impossible switches!
...
If you're interested only in switches to a particular tissue or isotype, you can specify this in the `testSP` function. Here, we only look at the proportion of switches that go to IGHA2:
